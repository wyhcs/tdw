<template>
  <section class="outline-panel">
    ...
    >
      <div slot-scope="{ data }" class="outline-node" :class="{ active: data.id === selectedId }">
        ...
        <span class="outline-actions">
          <el-button type="text" icon="el-icon-plus" :disabled="data.level >= 3" @click.stop="$emit('add-child', data)" />
...
<style scoped>
.outline-panel { height: 100%; display: flex; flex-direction: column; }
.panel-toolbar { height: 40px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #ebeef5; padding: 0 12px; }
.panel-title { font-weight: 600; color: #303133; }
.outline-empty { margin: 12px; }
.outline-tree { flex: 1; overflow: auto; padding: 8px 6px 12px; }
.outline-node { width: 100%; min-width: 0; display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.outline-label { min-width: 0; display: flex; align-items: center; gap: 6px; }
.outline-title { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.outline-actions { display: none; flex-shrink: 0; }
.outline-node:hover .outline-actions, .outline-node.active .outline-actions { display: inline-flex; }
.outline-actions .el-button { padding: 0 2px; }
