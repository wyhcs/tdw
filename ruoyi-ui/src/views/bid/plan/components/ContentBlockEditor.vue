  ...
  <section class="content-editor">
    ...
    <div v-else class="block-list">
      ...
      >
        ...
        <div v-if="block.contentType === 1" class="text-block">
          {{ parseContent(block).text || block.content }}
        </div>

        <div v-else-if="block.contentType === 2" class="table-block">
          <div class="block-subtitle">{{ parseContent(block).title || '表格内容' }}</div>
          <el-table :data="tableRows(parseContent(block))" size="mini" border>
            <el-table-column
              v-for="(header, headerIndex) in tableHeaders(parseContent(block))"
              :key="headerIndex"
        ...
        <div v-else class="diagram-block">
          <div class="block-subtitle">{{ parseContent(block).title || '结构图' }}</div>
          <p>{{ parseContent(block).description }}</p>
          <pre v-if="parseContent(block).mermaid">{{ parseContent(block).mermaid }}</pre>
          <el-image v-if="parseContent(block).imageUrl" :src="parseContent(block).imageUrl" fit="contain" />
        </div>
      </el-card>
    </div>
    ...
    <el-dialog :title="editForm.contentType === 1 ? '编辑文本块' : '编辑内容块 JSON'" :visible.sync="editVisible" width="680px" append-to-body>
      <el-alert
        v-if="editForm.contentType !== 1"
        title="内容块以 JSON 保存。文本、表格、结构图会按不同结构展示。"
        type="info"
        show-icon
        :closable="false"
      />
